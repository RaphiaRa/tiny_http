...
TH_LOCAL(th_err)
th_path_resolve_posix(th_str path, th_string* out)
{
    char in[TH_CONFIG_MAX_PATH_LEN + 1] = {0};
    size_t pos = 0;
    pos += th_fmt_strn_append(in, pos, sizeof(in) - pos, path.ptr, path.len);
    in[pos] = '\0';
    th_string_resize(out, TH_CONFIG_MAX_PATH_LEN, 0);
    char* out_ptr = th_string_at(out, 0);
    char* ret = realpath(in, out_ptr);
    if (ret == NULL)
        return TH_ERR_SYSTEM(errno);
    th_string_resize(out, strlen(out_ptr), 0);
    return TH_ERR_OK;
}
/*
TH_LOCAL(th_err)
th_path_resolve_against_posix(th_dir* dir, th_str path, th_string* out)
{
    char in[TH_CONFIG_MAX_PATH_LEN + 1] = {0};
    th_str root = th_dir_get_path(dir);
    size_t pos = 0;
    pos += th_fmt_strn_append(in, pos, sizeof(in) - pos, root.ptr, root.len);
    pos += th_fmt_str_append(in, pos, sizeof(in) - pos, "/");
    pos += th_fmt_strn_append(in, pos, sizeof(in) - pos, path.ptr, path.len);
    th_string_resize(out, TH_CONFIG_MAX_PATH_LEN, 0);
    char* out_ptr = th_string_data(out);
    char* ret = realpath(in, out_ptr);
    if (ret == NULL)
        return TH_ERR_SYSTEM(errno);
    th_string_resize(out, strlen(out_ptr), 0);
...
TH_LOCAL(th_err)
th_path_resolve_mock(th_str path, th_string* out)
{
    (void)path;
    th_string_clear(out);
    th_string_set(out, path);
    return TH_ERR_OK;
}
#endif
...
th_path_resolve_against(th_str path, th_dir* dir, th_string* out)
{
    ...
    pos += th_fmt_str_append(in, pos, sizeof(in) - pos, "/");
    pos += th_fmt_strn_append(in, pos, sizeof(in) - pos, path.ptr, path.len);
    return th_path_resolve(th_str_make(in, pos), out);
}

TH_PRIVATE(bool)
th_path_is_within(th_str realpath, th_dir* dir)
{
    th_str root = th_dir_get_path(dir);
    if (realpath.len < root.len)
        return false;
    return th_str_eq(th_str_make(realpath.ptr, root.len), root);
}
...
{
    size_t pos = 0;
    while ((pos = th_str_find_first(path, pos, '/')) != th_str_npos) {
        if (path.ptr[++pos] == '.')
            return true;
    }
    return false;
