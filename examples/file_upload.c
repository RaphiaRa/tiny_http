...
handler(void* userp, const th_request* req, th_response* resp)
{
    ...
    for (th_iter it = th_part_iter(req); th_next(&it);) {
        ...
        if ((err = th_save_to_disk(server, th_part_content(part), "upload_dir", filename)) != TH_ERR_OK) {
...
int main(void)
{
    ...
    if ((err = th_add_dir(server, "upload_dir", "./uploads")) != TH_ERR_OK)
        goto cleanup;
    if ((err = th_route(server, TH_METHOD_POST, "/", handler, server)) != TH_ERR_OK)
        goto cleanup;
    while (!stop) {
        th_poll(server, 1000);
    }
