...
typedef struct th_fake_socket_ops {
    ...
    char written[64];
    size_t written_len;
    size_t chunk_len; /* max bytes accepted per call across header+file; 0 = unlimited */
    th_err err;       /* returned once, then reset to TH_ERR_OK */
    size_t last_offset;
} th_fake_socket_ops;

static th_err
th_fake_sendfile(void* self, int fd, const th_iov* iov, size_t iovcnt, th_file* file, size_t offset, size_t len, size_t* result)
{
    (void)fd;
    (void)file;
    ...
    if (remaining > 0) {
        /* simulated file bytes: not copied anywhere, just counted */
...
th_fake_socket_ops_init(th_fake_socket_ops* ops)
{
    ...
    ops->base.recv = NULL;
    ops->base.sendfile = th_fake_sendfile;
    ops->written_len = 0;
    ops->chunk_len = 0;
    ops->err = TH_ERR_OK;
    ops->last_offset = 0;
...
TH_TEST_BEGIN(sendfile)
{
    ...
    TH_TEST_CASE_BEGIN(sendfile_no_header_sends_whole_file_in_one_call)
    {
        ...
        th_sendfile_op op;
        th_sendfile_op_init(&op, &socket, NULL, 0, &file, 0, 100, th_recorded_result_cb, &result);
        th_op_perform(&op.base);
        th_loop_run(&loop);

        TH_EXPECT(result.called);
        TH_EXPECT(result.err == TH_ERR_OK);
        TH_EXPECT(result.result == 100);
    }
    TH_TEST_CASE_END
    TH_TEST_CASE_BEGIN(sendfile_with_header_counts_header_bytes_toward_total)
    {
        th_fake_socket_ops_reset(&ops);

        th_iov iov[1] = {{(void*)"header", 6}};
        th_recorded_result result;
        th_recorded_result_init(&result);
        th_sendfile_op op;
        th_sendfile_op_init(&op, &socket, iov, 1, &file, 0, 100, th_recorded_result_cb, &result);
        th_op_perform(&op.base);
        th_loop_run(&loop);

        TH_EXPECT(result.called);
        TH_EXPECT(result.err == TH_ERR_OK);
        TH_EXPECT(result.result == 106);
        TH_EXPECT(memcmp(ops.written, "header", 6) == 0);
    }
    TH_TEST_CASE_END
    TH_TEST_CASE_BEGIN(sendfile_retries_when_header_partially_written)
    {
        th_fake_socket_ops_reset(&ops);
        ops.chunk_len = 3; /* only "hea" fits in the first call */
    ...
    TH_TEST_CASE_END
    TH_TEST_CASE_BEGIN(sendfile_advances_offset_by_file_bytes_sent_so_far)
    {
        th_fake_socket_ops_reset(&ops);
        ops.chunk_len = 10; /* forces at least two rounds for a 100-byte file */

        th_recorded_result result;
        th_recorded_result_init(&result);
        th_sendfile_op op;
        th_sendfile_op_init(&op, &socket, NULL, 0, &file, 50, 100, th_recorded_result_cb, &result);
        th_op_perform(&op.base);
        th_loop_run(&loop);
        ...
        TH_EXPECT(result.err == TH_ERR_OK);
        TH_EXPECT(result.result == 100);
        TH_EXPECT(ops.last_offset == 50 + 90); /* offset + file bytes sent before the last call */
    }
    TH_TEST_CASE_END
    TH_TEST_CASE_BEGIN(sendfile_eagain_submits_and_retries)
    {
        th_fake_socket_ops_reset(&ops);
        ...
        th_recorded_result_init(&result);
        th_sendfile_op op;
        th_sendfile_op_init(&op, &socket, NULL, 0, &file, 0, 10, th_recorded_result_cb, &result);
        th_op_perform(&op.base);
        th_loop_run(&loop);

        TH_EXPECT(result.called);
        TH_EXPECT(result.err == TH_ERR_OK);
        TH_EXPECT(result.result == 10);
    }
    TH_TEST_CASE_END
    TH_TEST_CASE_BEGIN(sendfile_error_completes_with_error)
    {
        th_fake_socket_ops_reset(&ops);
    ...
    TH_TEST_CASE_END
    TH_TEST_CASE_BEGIN(sendfile_abort_completes_with_given_error)
    {
        th_fake_socket_ops_reset(&ops);
