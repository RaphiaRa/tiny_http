...
typedef struct th_fake_socket_ops {
    ...
    char written[64];
    size_t written_len;
    size_t chunk_len; /* max bytes accepted per call; 0 = unlimited */
    th_err err;       /* returned once, then reset to TH_ERR_OK */
} th_fake_socket_ops;

static th_err
th_fake_send(void* self, int fd, const void* addr, size_t len, size_t* result)
{
    (void)fd;
    ...
    if (ops->chunk_len != 0 && n > ops->chunk_len)
        n = ops->chunk_len;
    memcpy(ops->written + ops->written_len, addr, n);
    ops->written_len += n;
    *result = n;
    return TH_ERR_OK;
}
...
TH_TEST_BEGIN(send)
{
    ...
    TH_TEST_CASE_END
    TH_TEST_CASE_BEGIN(send_eagain_submits_and_retries)
    {
        th_fake_socket_ops_reset(&ops);
    ...
    TH_TEST_CASE_END
    TH_TEST_CASE_BEGIN(send_error_completes_with_error)
    {
        th_fake_socket_ops_reset(&ops);
    ...
    TH_TEST_CASE_END
    TH_TEST_CASE_BEGIN(send_abort_completes_with_given_error)
    {
        th_fake_socket_ops_reset(&ops);
